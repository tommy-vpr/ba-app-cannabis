...
// interface CannabisCompanyContextValue {
//   companies: Company[];
//   filteredCompanies: Company[]; // derived
//   searchQuery: string;
//   setSearchQuery: (query: string) => void;
//   statusFilter: LeadStatusFilter;
//   setStatusFilter: (status: LeadStatusFilter) => void;
...
//   // Single derivation for filteredCompanies (status + search)
//   const filteredCompanies = useMemo(() => {
//     let list = companies;
//     if (statusFilter !== "All") {
//       const display = (s: Company["lead_status_l2"]) =>
...
//         const { companies: fetched, nextCursor } =
//           await getCannabisCompaniesPaginated(
//             12,
//             after,
//             userEmail,
//             statusFilter,
...
//       }
//     },
//     [userEmail, statusFilter, zipFilter] // <-- include zipFilter in deps
//   );
...
//   // Single effect: fetch when userEmail or statusFilter changes (page 1)
//   useEffect(() => {
//     if (!userEmail) return;

//     const zip = zipFilter.trim();
//     if (!isZipReady(zip)) return; // ignore partial zips like "94" or "1234"

//     const t = setTimeout(() => {
//       setCursors({ 1: null });
//       fetchCompaniesForPage(1, undefined);
//     }, 300); // small debounce so fast typing doesn't spam

//     return () => clearTimeout(t);
//   }, [userEmail, statusFilter, zipFilter, fetchCompaniesForPage]);

//   const toggleBookmark = async (companyId: string) => {
//     if (pendingId) return; // prevent double clicks across the grid
//     const target = companies.find((c) => c.id === companyId);
//     if (!target) return;
...
//         state: target.state ?? null,
//         zip: target.zip ?? null,
//         lead_status_l2: target.lead_status_l2 ?? null, // ✅ pass it
//       });
//     } catch (e) {
//       // revert on failure
//       setCompanies((prev) =>
